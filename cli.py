...
class Cli:
    ...
    def __define_parser() -> argparse.ArgumentParser:
        ...
        parser.add_argument("-e", "--env", action=UploadAction)
        parser.add_argument("-l", "--loud", help="Log everything about outgoing requests", action='store_true')
        ...
        payloads_parser.add_argument("-i", "--inpath", help="Path to trial csv data provided by PREC", type=pathlib.Path, required=True)
        payloads_parser.add_argument("-o", "--outpath", default="./json", help="Location to dump the payloads", type=pathlib.Path, required=True)
        payloads_parser.set_defaults(op=all_payloads)

        upload_parser = command_subparsers.add_parser('upload', help='Upload payloads to a variety testing environment')
        upload_parser.set_defaults(upload=True)
        upload_parser.add_argument("-i", "--inpath", help="Path to json payloads generated by the payloads command", type=pathlib.Path, required=True)
        upload_parser.add_argument('-y', '--year', help="The trial harvest year of this data", type=int)
        upload_parser.add_argument('-r', '--rewrite', help="Rewrite i.e. delete existing and write again", action='store_true', default=False)
